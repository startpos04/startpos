...
function verifyIntegration() {
  ...
  // Sidebar integration
  const sidebarCheck = checkFileContent('src/components/custom/dashboard/app-sidebar.tsx', [
    'BRANCH_VIEW_BILLING',
    'url: \'/billing\'',
    'CreditCardIcon',
  ])
  ...
  const billingSchemaCheck = checkFileContent('prisma/base/billing.prisma', [
  ...
  const coreSchemaCheck = checkFileContent('prisma/base/core.prisma', [
    'txQuotaLimit Int?',
  ...
  const permissionKeysCheck = checkFileContent('src/lib/authorization/permission-keys.ts', [
    'BRANCH_VIEW_BILLING',
    'BRANCH_MANAGE_BILLING',
  ])
  console.log(`   ${permissionKeysCheck.exists && Object.values(permissionKeysCheck.found).every(Boolean) ? '✅' : '❌'} Permission keys defined`)
  
  const rolePermissionsCheck = checkFileContent('src/lib/authorization/role-permissions.ts', [
    'BRANCH_VIEW_BILLING',
    'BRANCH_MANAGE_BILLING',
  ])
