 ...
 * use-hints.ts
 *
 * useHints(page) â€” fetches the next eligible hint for the current page and
...
export function useHints(page: string) {
  ...
  // Read display seconds from configs â€” cast from merged config
  ...
  useEffect(() => {
    ...
    fetchEligibleHint({ data: { page } })
      ...
      .catch(() => {
        // Non-fatal â€” hints are informational, never block the UI
      })
