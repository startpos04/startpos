 ...
 *
 * SubscriptionStatusVO — thin value-object wrapper around SubscriptionStatus
 * that adds domain methods for UI rendering: toLabel, toBannerSeverity, etc.
 *
 * Lives in platform (not in the app layer) because:
 *   - SubscriptionStatus is a platform type (entitlement-types.ts)
 *   - The sidebar and other platform UI components need it without importing
 *     from the app layer
 *
 * App-layer callers can re-export from this file or import directly.
 */

import { SubscriptionStatus } from './entitlement-types'
