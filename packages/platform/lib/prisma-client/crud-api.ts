...
export type CleanArgs<T extends ModelName, M extends DelegateMethods> = DeepStrip<Parameters<DB[T][M]>[0]>
export type InferResult<T extends ModelName, M extends DelegateMethods, A> = Prisma.Result<DB[T], A, M>

export type CrudProxy = {
  [K in ModelName]: <M extends DelegateMethods, A extends CleanArgs<K, M>>(action: M, args?: A) => Promise<import('neverthrow').Result<DeepPrettify<InferResult<K, M, A>>, string>>
}
