/**
 * permission-middleware.ts â€” Server-side permission enforcement middleware
 *
 ...
 *
 * Usage:
 *   import { requirePermission, requireAllPermissions, requireAnyPermission } from '@platform/lib/better-auth/permission-middleware'
 *   import { PermissionKeys } from '@platform/lib/authorization/permission-keys'
 *
 *   // Single permission
 ...
 *       // If the permission is denied, this handler is never called â€”
 ...
 *   - Pure middleware â€” no side effects beyond the permission check
...
// ---------------------------------------------------------------------------
// Permission error â€” thrown when the permission is denied
// ---------------------------------------------------------------------------
...
export function requirePermission(permission: PermissionKey) {
  return createMiddleware().server(async ({ next, context }) => {
    ...
    // -----------------------------------------------------------------------
    // Rebuild authorization summary from DB â€” not from the client session
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Pass authorization summary to the handler (optional â€” handler can use it)
    // -----------------------------------------------------------------------
    return next({
      context: {
        ...(context as object),
