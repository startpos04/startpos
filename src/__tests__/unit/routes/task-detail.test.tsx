 ...
 *
 * Strategy:
 *  - Mount via TaskDetailsDialog (overlay variant) to skip Route loader.
...
import { TaskDetailsDialog } from '@/routes/(private)/tasks/$taskId/index'
...
function renderDialog(taskId: string) {
  const onClose = vi.fn()
  return render(
    <TaskDetailsDialog open={true} onClose={onClose} taskId={taskId} />,
  )
}
...
describe('TaskDetailsDialog — loading / not-found', () => {
...
describe('TaskDetailsDialog — header', () => {
  ...
  it('renders task ID prefix (first 8 chars + "...")', async () => {
    ...
    renderDialog(task.id)
    await waitFor(() => {
      expect(screen.getByText(new RegExp(`ID: ${task.id.slice(0, 8)}`))).toBeInTheDocument()
    })
  })
})
...
describe('TaskDetailsDialog — tabs', () => {
  it('renders "Task Details" and "Timeline & Logs" tabs', async () => {
    const task = makeTask({ status: 'PENDING' })
    vi.mocked(fetchTasks).mockReturnValue({ data: [task], isLoading: false } as any)
    renderDialog(task.id)
    await waitFor(() => {
...
describe('TaskDetailsDialog — action buttons', () => {
