...
import { makeId, makeInventoryRecord, makePosProduct, makePosVariant, seedMockUser, resetMockUser } from '@/lib/__tests__/helpers'
...
// ---------------------------------------------------------------------------
// Mock: overlay — inline factory avoids hoisting issues
// ---------------------------------------------------------------------------
...
describe('ProductCard — click behaviour', () => {
  it('calls showModal with ProductDialog when in stock and card is clicked', () => {
    const { product } = makeProduct({ stock: 5 })
    const { container } = render(<ProductCard cartItems={emptyCart} product={product} onAdd={onAdd} />)
  ...
  it('passes product and cartItems to ProductDialog via showModal options', () => {
    const { product } = makeProduct({ stock: 5 })
    const cart: posItem[] = []
    render(<ProductCard cartItems={cart} product={product} onAdd={onAdd} />)
    fireEvent.click(screen.getByText(product.name))
    const [, options] = vi.mocked(showModal).mock.calls[0] as any[]
  ...
  it('does NOT call showModal when out of stock', () => {
    const { product } = makeProduct({ stock: 0 })
    const { container } = render(<ProductCard cartItems={emptyCart} product={product} onAdd={onAdd} />)
    fireEvent.click(container.firstElementChild!)
  ...
  it('does not call showModal when cart already consumes all stock', () => {
