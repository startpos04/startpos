...
describe('POS page — session guard', () => {
  it('calls showModal(OpenSessionDialog) when vendorSession is null', async () => {
    seedMockUser({ vendorSession: null } as any)
    renderPosPage()
    await waitFor(() => {
      expect(vi.mocked(showModal)).toHaveBeenCalledWith(OpenSessionDialog)
    })
  })

  it('calls showModal(OpenSessionDialog) when session status is CLOSED with verifiedCash set', async () => {
  ...
  it('calls showModal(AlertPrompt) when session is CLOSED with verifiedCash=null', async () => {
    seedMockUser({
      vendorSession: makeVendorSession({
        status: SessionStatus.CLOSED,
        verifiedCash: null,
  ...
  it('does NOT call showModal when session is OPEN', async () => {
    seedMockUser({
      vendorSession: makeVendorSession({ status: SessionStatus.OPEN }),
    } as any)
    renderPosPage()
