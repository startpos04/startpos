      ...
      for (const sub of subscriptions) {
        ...
        if (providerId) {
          try {
            ...
            warnings.push(`Business ${sub.businessId}: Failed to get provider info for ${providerId}: ${error instanceof Error ? error.message : String(error)}`)
          }
        }

        // Build provider-specific message and action link
        const { title, message, link } = buildReminderContent(
          windowDays,
    ...
    switch (providerId) {
      case 'stripe':
        if (providerCapabilities?.supportsRecurring) {
          message += ' Your payment method will be charged automatically. You can update your billing details or view payment history in your billing dashboard.'
