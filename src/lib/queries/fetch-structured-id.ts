...
export async function fetchStructuredId(type: SequenceType) {
  ...
  // 1. Create the ID
  const counterId = `${user.organization.id}-${user.branch.id}-${type}-${year}-${month}-${day}`
  ...
  try {
    ...
    if (existing) {
      // 3. Update existing
      await sequenceCounterCollection.update(counterId, draft => {
        draft.lastNumber += 1
      })
      ...
      // 4. Insert new
      await sequenceCounterCollection.insert({
        id: counterId,
        type,
        year,
        month,
    ...
    throw new Error('Failed to generate structured ID. Please try again.')
  }
}
