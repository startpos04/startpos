...
export const InventoryPolicy = {
  ...
  validateDeduction(
    ...
    if (resultingQuantity < 0) {
      throw new InsufficientStockError(`Cannot deduct ${requestedQuantity} from ${currentQuantity} for ${productName || variantId}`, {
        variantId,
   ...
   * ```
   */
  canDeduct(currentQuantity: number, requestedQuantity: number, inventoryMode: InventoryMode): boolean {
   ...
   */
  validateBatchTransfer(sourceBatchId: string, sourceQuantity: number, transferQuantity: number, inventoryMode: InventoryMode, productName?: string): void {
    this.validateDeduction(sourceBatchId, sourceQuantity, transferQuantity, inventoryMode, productName, sourceBatchId)
  },
  ...
  validateProductionConsumption(
    ...
    this.validateDeduction(materialId, availableQuantity, requiredQuantity, inventoryMode, materialName)
  },
   ...
   */
  validateWasteDisposal(variantId: string, availableQuantity: number, wasteQuantity: number, productName?: string): void {
    ...
    if (wasteQuantity > availableQuantity) {
      throw new InsufficientStockError(`Cannot dispose ${wasteQuantity} when only ${availableQuantity} exists`, {
        variantId,
