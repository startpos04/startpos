...
export const BranchValidationEngine = {
   ...
   */
  getCurrentBranchUsage(
    businessId: string,
    branchId: string,
    usageCounterCollection: Map<string, any>,
    ...
    const branchCounter = [...usageCounterCollection.values()].find(
      c =>
        ...
        c.branchId === branchId &&
        !c.isClosed &&
        new Date(c.billingPeriodStart).getTime() === currentPeriodStart.getTime(),
    )
  ...
  getBranchCreditBalance(
    creditLedgerCollection: Map<string, any>,
    businessId: string,
    branchId: string,
  ): BranchCreditSnapshot {
