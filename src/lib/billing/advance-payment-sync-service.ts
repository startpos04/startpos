...
export class StripeAdvancePaymentSyncService extends BaseAdvancePaymentSyncService {
  ...
  async syncAdvancePayment(payment: BillingPayment): Promise<SyncResult> {
    ...
    try {
      ...
      const schedule = await this.createSubscriptionSchedule(
        subscription,
        payment
      )
  ...
  private async createSubscriptionSchedule(
    ...
    const advanceExpiresAt = Math.floor(
      (payment.advancePaymentExpiresAt?.getTime() || Date.now()) / 1000
    )
 ...
 */
export function createAdvancePaymentSyncService(
