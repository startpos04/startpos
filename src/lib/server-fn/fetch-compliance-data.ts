  ...
  try {
    ...
    if (countryCode === 'PH') {
      ...
      formData = {
        ...
        vatRegistrationDate: phCompliance?.vatRegistrationDate ? new Date(phCompliance.vatRegistrationDate).toISOString().split('T')[0] : '',

        // Business Permits
        ptuNumber: phCompliance?.birPtuNumber ?? '',
        ptuIssueDate: phCompliance?.birPtuIssuedAt ? new Date(phCompliance.birPtuIssuedAt).toISOString().split('T')[0] : '',
