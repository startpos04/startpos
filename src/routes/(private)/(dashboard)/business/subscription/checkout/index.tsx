...
function OrderSummary({
  ...
  const dueNow = interval === 'annual'
    ? annualTotal(plan)
    : paymentType === 'manual'
      ? plan.monthlyPrice * manualPeriods
      : plan.monthlyPrice
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between rounded-lg bg-muted/50 border px-3 py-2.5">
        ...
        <Link
          to="/business/subscription/plans"
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        <div className="rounded-lg border bg-background px-3 py-3.5 focus-within:ring-1 focus-within:ring-primary transition min-h-[46px]">
          <CardElement options={cardOptions} onChange={e => { setCardComplete(e.complete); setCardError(e.error?.message ?? null) }} />
        </div>
        {cardError && (
      ...
      <div className="flex items-start gap-2 rounded-lg bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/50 p-3">
        ...
        <p className="text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
          Powered by Stripe. Your card is encrypted and never stored on our servers.
        </p>
      </div>
...
function ManualPaymentForm({
  ...
  const totalCents = interval === 'annual'
    ? annualTotal(plan)
    : plan.monthlyPrice * periods
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault()
    if (!proofImage) { toast.error('Please upload proof of payment'); return }
    mutation.mutate({
      data: {
        planId,
        amount: totalCents,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {manualConfig && (
        <div className="rounded-lg bg-muted/60 border p-3 text-sm space-y-1">
          ...
          <p><span className="text-muted-foreground">Account name: </span>{String(manualConfig.config.accountName ?? '—')}</p>
          <p><span className="text-muted-foreground">Account number: </span>{String(manualConfig.config.accountNumber ?? '—')}</p>
          {manualConfig.config.paymentInstructions && (
            <p className="text-muted-foreground text-xs pt-1">{String(manualConfig.config.paymentInstructions)}</p>
          )}
        </div>
      )}
  ...
  if (!planId) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <Button asChild variant="outline"><Link to="/business/subscription/plans">Back to plans</Link></Button>
      </div>
    )
  }
      ...
      <div className="w-full px-4 pb-6 space-y-5">
        ...
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" asChild className="shrink-0">
            <Link to="/business/subscription/plans"><ArrowLeftIcon className="h-4 w-4" /></Link>
          </Button>
          <div>
          ...
          <div className="flex flex-col items-center text-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                You can't submit a new payment while one is being reviewed.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-lg border bg-muted/40 divide-y text-sm">
            <div className="flex items-center justify-between px-4 py-3">
              ...
              <span className="font-semibold">
                ₱{((pendingPayment.amount ?? 0) / 100).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
              </span>
            </div>
            <div className="flex items-center justify-between px-4 py-3">
              ...
              {paymentType === 'stripe' ? (
                stripePromise ? (
                  ...
                  <div className="flex flex-col items-center gap-3 py-8 text-center">
                    ...
                    <p className="text-sm font-medium">Stripe not configured</p>
                    <p className="text-xs text-muted-foreground">Add <code className="bg-muted px-1 rounded">VITE_STRIPE_PUBLIC_KEY</code> to .env and rebuild.</p>
                  </div>
                )
              ) : (
                <ManualPaymentForm
