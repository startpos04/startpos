...
import { Dialog, DialogClose, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@startpos-core/components/ui/dialog'
...
export function AuthPrompt({ open, onClose, onConfirm }: AuthPromptProps) {
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      const auth = await AuthEngine.authorizeFeature(value)
      form.reset()
      if (!auth) return
