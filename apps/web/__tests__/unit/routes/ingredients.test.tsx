...
describe('Ingredients page — add ingredient', () => {
  it('clicking Add Ingredient calls MountManager.show', async () => {
    vi.mocked(fetchIngredients).mockReturnValue({ data: [], isLoading: false } as any)
    renderIngredientsPage()
...
describe('Ingredients page — edit ingredient', () => {
  it('clicking edit button on a row calls MountManager.show', async () => {
...
describe('Ingredients page — delete handler', () => {
  it('clicking delete (trash) button calls MountManager.show(WarningPrompt)', async () => {
    ...
    await waitFor(() => {
      expect(vi.mocked(MountManager.show)).toHaveBeenCalledWith(
        WarningPrompt,
...
describe('Ingredients page — restock handler', () => {
  it('clicking restock (database) button calls MountManager.show', async () => {
    ...
    if (allBtns.length >= 2) {
      ...
      // Fallback: just assert MountManager.show was not called yet (no crash)
