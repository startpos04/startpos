...
export const createStripeSubscription = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    let stripeCustomerId = business.preferredPaymentProvider === 'STRIPE' ? business.externalCustomerId : null
    ...
    // 6. Calculate price
    const unitAmount = billingInterval === 'annual' ? plan.annualPrice || plan.monthlyPrice * 12 : plan.monthlyPrice
