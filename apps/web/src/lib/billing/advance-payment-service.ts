...
export class AdvancePaymentService {
  ...
  async applyAdvancePayment(payment: BillingPayment): Promise<{
    ...
    try {
      ...
      const { subscription } = await syncService.applyAdvanceCredits(payment.subscriptionId, payment)
   ...
   */
  async shouldSkipBillingPeriod(subscriptionId: string, periodStart: Date): Promise<boolean> {
