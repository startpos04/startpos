...
### Phase B — Integrity and Security
...
| Task | Action | Justification | Status |
|------|--------|---------------|--------|
| **B1** | Add server-side task transition validator — a server function that imports `checkWorkflowPermission` from `task-workflow.ts` and validates before committing | Phase 1 Part 10.6: all auth is client-side only | ⬜ Pending |
| **B2** | Add status guard to `EditTaskSidebar` — reject edits on FULFILLED, REVIEWED, CANCELLED | Phase 1 Part 10.1: no edit guard exists | ⬜ Pending |
| **B3** | Add role+status guard to task delete — ADMIN/SUPERVISOR only, DRAFT/PENDING only | Phase 1 Part 10.1: no delete guard exists | ⬜ Pending |
| **B4** | Fix `ReconcileNow` — set task to FULFILLED instead of REVIEWED directly | Phase 1 Part 10.5: reconciliation lifecycle inconsistency | ⬜ Pending |
| **B5** | Fix `ReconcileLater` — capture `verifiedCash` and `variance` at submission | Phase 1 Part 10.5: verifiedCash never set in this path | ⬜ Pending |
...
| **B7** | Add missing target-branch IN movement for BRANCH_TRANSFER tasks | Phase 1 Part 10.4: branch transfers deduct source but never credit destination | ⬜ Pending |

**Note on B1:** The server function imports `checkWorkflowPermission` directly from the existing `task-workflow.ts`. It does not reimplement the logic. This preserves the single-source-of-truth for transition rules.
...
|------|--------|---------------|
| **C1** | Change task creation default to DRAFT; add "Submit for Approval" action | Phase 1 Part 10.1: DRAFT status declared but never produced |
| **C2** | Surface `suggestedQty`, `approvedQty`, `verifiedQty` as distinct UI fields (they already exist as schema columns) | Phase 1 Part 10.1: three columns exist but all collapse to same value |
| **C3** | Allow approver to modify `approvedQty` at approval time | Business Invariant TASK-3 |
| **C4** | Clerk records `verifiedQty` at fulfillment; `InventoryEngine.applyTaskFulfillment` uses `verifiedQty` | Business Invariant TASK-3; requires Phase A complete |
| **C5** | Complete Timeline tab — add `fulfilledAt`, `reviewedAt`, `canceledAt` events | Phase 1 Part 10.1: timeline is incomplete |
| **C6** | Send `TASK_ASSIGNED` notification when `clerkId` is set | Phase 1 Part 10.3: type declared, never used |
...
**New abstraction activated: `workflow.ts` (via `purchaseWorkflow`)**
Justified at this phase by the existence of two workflows needing the same pattern. See ADR-002.

| Task | Action | Justification |
|------|--------|---------------|
| **D1** | Add `status` column to Purchase (DRAFT, PENDING_APPROVAL, APPROVED, RECEIVED, VOIDED, CLOSED). Existing records → RECEIVED | Phase 1 Part 10.2: no status field; void via notes prefix |
| **D2** | Change void to set `status = VOIDED`; retain `notes` prefix during transition window | Phase 1 Part 9.3: void detection fragility |
| **D3** | Define `purchaseWorkflow` using `createWorkflow()` from `workflow.ts` | **First second consumer of workflow.ts — justifies the framework** |
| **D4** | Migrate `task-workflow.ts` to use `createWorkflow()` in same PR as D3 | Justified now that two consumers exist simultaneously |
| **D5** | Introduce Purchase Request path: create purchase at PENDING_APPROVAL status | New capability; existing quick-receive path unchanged |
| **D6** | Implement configurable approval gate for purchases above threshold | Phase 2 Business Invariant PUR-1 |
| **D7** | Add `PURCHASE_PENDING_APPROVAL` notification | Phase 1 Part 10.3: missing notification type |

**Note on D3/D4:** These are intentionally in the same phase. Migrating `task-workflow.ts` alone (without a second consumer existing) is unjustified. Migrating both simultaneously is justified because both patterns now share the same framework.

---

### Phase E — Receiving Domain (Future)
**Goal:** Introduce the GRN concept — the bridge between supplier delivery and inventory credit.

**New abstraction introduced: `GoodsReceipt` model and Receiving domain**
This is the highest-effort, lowest-immediate-risk phase. It introduces a new domain concept not currently expressible in the existing architecture.

| Task | Action |
|------|--------|
| **E1** | Introduce `GoodsReceipt` model linked to Purchase |
| **E2** | Add "Receive Goods" action on an approved purchase |
| **E3** | GoodsReceipt confirmation triggers `InventoryEngine.applyPurchaseReceipt()` (instead of purchase creation) |
| **E4** | Quick-receive path remains for cash-and-carry operations (backward compatible) |

**Precondition:** Phase D complete. Purchase status lifecycle must exist before GRN can be meaningfully linked to it.

---

### Phase F — Entitlement Maturity (Parallel)
...
| Task | Action |
|------|--------|
| **F1** | Introduce `BusinessSubscription` model |
| **F2** | Wire real subscription data into `getAuthUser` entitlement assembly |
| **F3** | Remove `ENABLE_TASK` SystemConfig; gate through EntitlementEngine only |
...
Phase B — Integrity & Security guards          🔶 IN PROGRESS — B6 done; B1–B5, B7 pending
Phase C — Task model maturity                  ⬜ Pending [minor schema changes]
Phase D — Purchase status + workflow adoption  ⬜ Pending [1 schema migration]
Phase F — Entitlement go-live                  ⬜ Pending [parallel, billing-driven]
Phase E — Receiving domain                     ⬜ Pending [long-term, new domain]
```
